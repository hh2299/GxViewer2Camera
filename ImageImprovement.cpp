...
*/
//----------------------------------------------------------------------------------
void CImageImprovement::GetDialogInitParam(GX_DEV_HANDLE hDeviceHandle1, GX_DEV_HANDLE hDeviceHandle2)
{
    ...
    emStatus = GXIsImplemented(m_hDevice1, GX_INT_CONTRAST_PARAM, &bContrast);
    GX_VERIFY(emStatus);
    if (bContrast)
    {
        // Get contrast adjustment parameters
        emStatus = GXGetInt (m_hDevice1, GX_INT_CONTRAST_PARAM, &i64ContrastParam);
        GX_VERIFY(emStatus);
    ...
    emStatus = GXIsImplemented(m_hDevice1, GX_INT_COLOR_CORRECTION_PARAM, &bColorCorrection);
    GX_VERIFY(emStatus);
    if (bColorCorrection)
    {
        // Get color correction adjustment parameters
        emStatus = GXGetInt (m_hDevice1, GX_INT_COLOR_CORRECTION_PARAM, &i64ColorCorrection);
        GX_VERIFY(emStatus);
    }

    bool bGamma = false;
    emStatus = GXIsImplemented(m_hDevice1, GX_FLOAT_GAMMA_PARAM, &bGamma);
    GX_VERIFY(emStatus);
    if (bGamma)
    {
        // Get Gamma parameter
        emStatus = GXGetFloat(m_hDevice1, GX_FLOAT_GAMMA_PARAM, &dGammaParam);
        GX_VERIFY(emStatus);
...
void CImageImprovement::on_ColorCorrect_clicked(bool bChecked)
{
    ...
    if (bChecked)
    {
        ...
        emStatus = GXGetInt(m_hDevice1, GX_INT_COLOR_CORRECTION_PARAM, &m_i64ColorCorrection);
